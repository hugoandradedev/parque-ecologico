...
class Router {
    ...
    public static function route() {
        ...
        $router->get('/api/visitas/listar', 'VisitaTecnicaController@index', 'AuthMiddleware');
        $router->post('/api/visitas/aprovar/{id}', 'VisitaTecnicaController@aprovar', 'AuthMiddleware');
        $router->post('/api/visitas/rejeitar/{id}', 'VisitaTecnicaController@rejeitar', 'AuthMiddleware');
        $router->post('/api/visitas/excluir/{id}', 'VisitaTecnicaController@delete', 'AuthMiddleware');
        ...
        $router->post('/api/contato/excluir/{id}', 'ContatoController@delete', 'AuthMiddleware');
    ...
    public function dispatch() {
        ...
        http_response_code($pathExists ? 405 : 404);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(["erro" => $pathExists ? "Método não permitido" : "Rota não encontrada"]);
    }
   ...
   private function handleException(Throwable $e, $uri) {
    ...
    error_log("Unhandled error on {$uri}: " . $e->getMessage());
