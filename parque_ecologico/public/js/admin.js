...
const VISITA_API = "/parque_ecologico/api/visitas";
...
async function aprovar(id, tipo = null) {

    if (!confirm("Aprovar?")) return;

    const baseUrl = getRegistroApiBase(id, tipo);
    await fazerRequisicao(`${baseUrl}/aprovar/${id}`);
    atualizarCardStatus(id, "aprovado", tipo);
}

async function rejeitar(id, tipo = null) {

    if (!confirm("Rejeitar?")) return;

    const baseUrl = getRegistroApiBase(id, tipo);
    await fazerRequisicao(`${baseUrl}/rejeitar/${id}`);
    atualizarCardStatus(id, "rejeitado", tipo);
}

async function excluir(id, tipo = null) {

    if (!confirm("Excluir?")) return;

    const baseUrl = getRegistroApiBase(id, tipo);
    await fazerRequisicao(`${baseUrl}/excluir/${id}`);
    removerCard(id, tipo);
}

function getRegistroApiBase(id, tipo = null) {
    const card = getRegistroCard(id, tipo);
    const registroTipo = tipo || card?.dataset.tipo;

    if (registroTipo === "visita") return VISITA_API;
    if (registroTipo === "contato") return MENSAGENS_API;

    return API;
}

function atualizarCardStatus(id, status, tipo = null) {
...
function removerCard(id, tipo = null) {

    getRegistroCard(id, tipo)?.remove();
}

function getRegistroCard(id, tipo = null) {
    const selector = tipo
